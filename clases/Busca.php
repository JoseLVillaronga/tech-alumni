...
class Busca
{
	...
	static function buscaItemLoteStock($s){
		...
		if(ctype_xdigit($s) AND strlen($s)=="12"){
			$query="SELECT stock.stk_id,lstk_serie FROM stock,lote_stock 
					...
					AND stk_activo = 1
					AND UPPER(lstk_mac) = '".Funcion::textoStandard($s)."'
					AND ss_id NOT IN (5)
					ORDER BY lstk_id DESC";
			$query2="SELECT stock.stk_id,lstk_serie FROM stock,lote_stock 
					WHERE stock.stk_id=lote_stock.stk_id
					AND stk_activo = 1
					AND UPPER(lstk_serie) = '".Funcion::textoStandard($s)."'
					AND ss_id NOT IN (5)
					ORDER BY lstk_id DESC";
			$res=Db::listar($query);
			if(count($res)=="0"){
	...
	static function buscaItemLoteStockBaja($s){
		$s=str_replace("'", "\'", $s);
		if(ctype_xdigit($s) AND strlen($s)=="12"){
		$query="SELECT stock.stk_id,lstk_serie,stk_activo FROM stock,lote_stock 
				WHERE stock.stk_id=lote_stock.stk_id
				AND lstk_id IN (SELECT lstk_id FROM lote_stock 
				WHERE lstk_id IN (SELECT lstk_id FROM lote_stock AS ls,stock AS s WHERE s.stk_id = ls.stk_id AND stk_activo = 2 AND lstk_mac = '$s')
				OR lstk_id IN (SELECT lstk_id FROM lote_stock AS ls,stock AS s WHERE s.stk_id = ls.stk_id AND ss_id IN (4,5) AND lstk_mac = '$s'))
				ORDER BY lstk_id DESC LIMIT 1";
		$query2="SELECT stock.stk_id,lstk_serie,stk_activo FROM stock,lote_stock 
				WHERE stock.stk_id=lote_stock.stk_id
				AND lstk_id IN (SELECT lstk_id FROM lote_stock 
				WHERE lstk_id IN (SELECT lstk_id FROM lote_stock AS ls,stock AS s WHERE s.stk_id = ls.stk_id AND stk_activo = 2 AND lstk_serie = '$s')
				OR lstk_id IN (SELECT lstk_id FROM lote_stock AS ls,stock AS s WHERE s.stk_id = ls.stk_id AND ss_id IN (4,5) AND lstk_serie = '$s'))
				ORDER BY lstk_id DESC LIMIT 1";
			$res=Db::listar($query);
			if(count($res)=="0"){
				$res=Db::listar($query2);
	...
	static function buscaItemLoteStockPorMAC($s){
		$s=str_replace("'", "\'", $s);
		if(empty($s)){return;}
		$queryRem="SELECT stk_id,lstk_id FROM lote_stock WHERE UPPER(lstk_mac) = '".Funcion::textoStandard($s)."' ORDER BY lstk_id DESC LIMIT 1";
		$nRem=Db::listar($queryRem);
		if(empty($nRem[0]['stk_id'])){
			return;
		}else{
