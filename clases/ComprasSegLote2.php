...
class ComprasSegLote2
{
	...
	public function __construct($c=null){
		if(is_null($c)){
			...
			$query="SELECT * FROM lote_compras_sugerencias WHERE scomp_id = $c";
			$res=Db::listar($query);
			if(count($res) != "0"){
				foreach($res as $fila){
					$this->lote[]=array(
						"compsl_id"=>$fila['compsl_id'],
						"scomp_id"=>$fila['scomp_id'],
	...
	public function actualizaDb(){
		...
		$query="UPDATE `lote_compras_sugerencias`
				SET
				`compsl_id` = :id,
				`scomp_id` = :idCS,
				`comp_id` = :repuesto,
	...
	public function agregaADb(){
		$con=Conexion::conectar();
		$repu=$this->getRepuestoId();
		$query="INSERT INTO `lote_compras_sugerencias`
				(`compsl_id`,
				`scomp_id`,
				`comp_id`,
				`compsl_cantidad`,
	...
	public function borraPorId($ID){
		...
		$con=Conexion::conectar();
		$query = "DELETE FROM lote_compras_sugerencias WHERE compsl_id = :id";
		$stmt = $con -> prepare($query);
