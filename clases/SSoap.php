...
class SSoap{
	...
	public function handle(){
		...
		if($this->wsdl_out==true){
			...
			$server = new SoapServer($tmpfile, array('cache_wsdl' => WSDL_CACHE_NONE));
	...
	private function calculateSoapAddress(){
		...
		if($_SERVER['REMOTE_ADDR']=="192.168.1.1" OR $_SERVER['REMOTE_ADDR']=="192.168.3.1"){
			$port=":10080";
		}else{
			$port="";
		}
		$address="$SCHEME://$SERVER_NAME$port$SCRIPT_NAME";
		return $address;
	}
	private	function echoXML($oh){
		...
		echo '<link rel="icon" href="../imagenes/favicon.ico" type="image/x-icon">';
		echo '<style type="text/css">';
		echo ' body { font-family: arial; color: #000000; background-color: #FBFDFC;background-image: url(../imagenes/main_bg.jpg);background-repeat:repeat-y;background-position: center top; margin: 0px 0px 0px 0px; }';
		...
		echo '.salam {float: right;font-family: tahoma;font-size: 9px; padding-right: 20px;color:rgba(2,66,94,.01); }';
		echo ' li { list-style-type: none; margin-top: 10px; color: #000000; }';
		echo '.content{margin-left: 0px; padding-bottom: 2em; }';
		echo '.nav {padding-top: 10px; padding-bottom: 10px; padding-left: 15px; font-size: .70em;margin-top: 10px; margin-left: 0px; color: #000000;background-color:rgba(4,4,180,.6); width: 20%; margin-left: 20px; margin-top: 20px; }';
		echo ' .title {font-family: arial; font-size: 26px; font-weight:800; color: #ffffff;background-color: red; width: 105%; margin-left: 0px;padding-top: 10px; padding-bottom: 10px; padding-left: 25px;}';
		echo '.hidden {position: absolute; visibility: hidden; z-index: 200; left: 250px; top: 100px;font-family: arial; overflow: hidden; width: 600;padding: 20px; font-size: 10px; background-color: rgba(161,121,13,.9);layer-background-color:#FFFFFF; }';
		...
		echo '</script>';
		echo '</head><body><div class="content"><br><br><div class="title">'.$tns.'</div>';
		echo '<div class="salam">SSoap by S.Maroofi ( maroofi at gmail com ) , M.Dastpak ( hdbplus.md at gmail com), Modified by José Luis Villaronga (jlvillaronga@teccamsrl.com.ar)</div>';
		echo '<div class="nav"><p>Ver el <a href="'.$address."?wsdl".'">WSDL</a> para el servicio. Haga click sobre los nombres de las operaciones para verlas en detalle.';
		echo '</p>';
		...
		if(empty($operation)==false){
			...
			foreach($operation as $operate){
				...
				echo '<font color="#ffffff">Cerrar</font>';
				echo '</a><br><br>';
				echo '<font color="white">Nombre :&nbsp;&nbsp;&nbsp;</font>';
				echo isset($operate->name)?$operate->name:'NoName';
				echo '<br>';
				echo '<font color="white">Documentación :&nbsp;&nbsp;&nbsp;</font>';
